...
<script>
    $(document).ready(function() {
        ...
        $('.edit-btn').click(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#edit_tendonvi').val(response.donvi.tendonvi);
                    $('#edit_tenviettat').val(response.donvi.tenviettat);
                    $('#edit_maloai').val(response.donvi.maloai);
                    // Cập nhật action của form
                    $('#editForm').attr('action', '{{ route("donvi.update", ":id") }}'.replace(':id', id));
                },
                error: function(xhr) {
        ...
        $('.dataTables-donvi').DataTable({
            ...
            columnDefs: [{
                orderable: false,
                targets: [0, 4], // Không sắp xếp cột STT và Thao tác
